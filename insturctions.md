  ...
  <head>
    ...
    <style>
      :root {
        ...
        --bp-compact: 520px;
        --bp-small: 600px;
        --focus-ring: 0 0 0 4px rgba(124, 58, 237, 0.14);
        --accent-1: #6ee7ff;
        --accent-2: #a78bfa;
        --accent-3: #22d3ee;

        /* Susie: maroon/crimson */
        --susie: #9f1239;
      ...
      * {
      ...
      .card {
        ...
        overflow: hidden;
        box-shadow: 0 8px 20px rgba(1, 5, 18, 0.5);
        display: flex;
        flex-direction: column;
        min-width: 0;
      }
...
/*         width: min(740px, calc(100vw - 24px)); */
      ...
      /* Preferences: label then select on next line */
      .pref-row {
        display: grid;
        gap: 6px;
        align-items: start;
      ...
      /* No results block (UX tweak: text left, +New right) */
      .no-results {
      ...
      .chat-sheet {
        ...
        max-height: 86dvh;
        font-family: var(--chat-font); /* chat-only font */
      }
      ...
      .chat-composer {
        ...
        gap: 8px;
        align-items: stretch; /* makes send match input height */
        margin-top: 10px;
      }
      ...
      /* Send button: match textarea height */
      #chat-send {
        width: 50px;
        height: 42px;
        justify-content: center;
      ...
      /* Bubble shrink-to-fit instead of stretching full width */
      .bubble {
        background: #12182a;
        border: 1px solid rgba(255, 255, 255, 0.08);
        border-radius: 10px;
      ...
      /* Offline modal: make it stand out more */
      #offline-modal {
  ...
  <body>
          ...
          <div class="menu-row">
            <div class="menu-item">
              <div class="menu-kv">
                ...
                <span class="menu-note" id="m-chat-online">(0 online)</span>
              </div>
        ...
        <div class="account-pref">
          ...
          <div class="pref-row">
            ...
            <select id="pref-cover" class="select">
              <option value="ANILIST">AniList (Default — Recommended, high quality)</option>
              <option value="JIKAN">Jikan (MAL) (Larger DB — lower quality / sometimes inaccurate)</option>
              <option value="KITSU">Kitsu (High quality — often inaccurate)</option>
            </select>
          </div>
        </div>
    ...
    <main class="container" id="main">
      <div class="toolbar">
        <div class="toolbar-top">
          <div class="search-container">
            <input
              ...
              aria-label="Search titles (press / to focus)"
            />
        ...
        <div class="toolbar-top">
          ...
          <div class="view-toggle" role="group" aria-label="View mode">
            <button class="seg" id="view-btn-grid" aria-pressed="true" title="Grid">
              <i class="fa-solid fa-grip"></i>
            </button>
            <button class="seg" id="view-btn-compact" aria-pressed="false" title="Shelf">
      ...
      >
        <div class="muted small">
          Pro tip: press / to search. Click -/+ to adjust EP (Shift = ±5). Use the gear to edit, trash to delete.
        </div>
        ...
        <div class="form">
          ...
          <div class="row">
            ...
            <div style="margin-left: auto">
              <button class="btn danger" id="btn-delete" style="display: none">
          ...
          <div class="hint">
            Covers are stored in your DB for sync. Background cover fetching respects per-anime source overrides.
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="modal" id="auth-modal" role="dialog" aria-modal="true" aria-hidden="true">
      <div class="sheet" role="document" style="max-width: 560px">
        ...
        <div class="form" id="signin-form">
          ...
          <!-- Forgot password link (requested style/placement) -->
          <div class="row" style="margin-top: -4px">
            <a class="text-link" id="forgot-password" href="javascript:void(0)">Forgot password?</a>
          </div>
        ...
        <div class="form hidden" id="signup-form">
          ...
          <div class="row" style="margin-top: -4px">
            <a class="text-link" id="resend-confirmation" href="javascript:void(0)">Resend confirmation email</a>
          </div>
    ...
    <!-- Email Verified Modal (static: cannot exit unless reload) -->
    <div
      class="modal"
      id="verify-modal"
      role="dialog"
    ...
    <div class="modal" id="offline-modal" role="dialog" aria-modal="true" aria-hidden="true" data-static="true">
      <div class="sheet" role="document" style="max-width: 520px">
        <h3><i class="fa-solid fa-wifi"></i> You’re offline</h3>
        <p class="muted-small">
          Your connection dropped. AniMehList will sync again automatically when you’re back online.
        </p>
        <p class="hint">Tip: keep this tab open — no need to reload.</p>
      </div>
    </div>
    ...
    <!-- Import Modal (requested redesign) -->
      ...
      <div class="sheet" role="document" style="max-width: 640px">
        ...
        <div class="form">
          ...
          <div class="upload-zone">
            ...
            <button class="btn ghost" id="import-browse">
          ...
          <div class="hint">We’ll map statuses/kinds and fetch missing covers in the background.</div>
        </div>
      </div>
    </div>
      ...
      <div class="sheet chat-sheet" role="document">
        ...
        <div id="chat-messages" class="chat-body" role="log" aria-live="polite">
          <div class="chat-loading" id="chat-loading">
            <i class="fa-solid fa-spinner" style="font-size: 20px" class="spin"></i>
          </div>
          ...
          <div class="chat-typing" id="chat-typing">Someone is typing…</div>
        </div>

        <div class="chat-composer">
          <img id="chat-self-avatar" class="chat-avatar" alt="" />
          <textarea id="chat-input" class="input chat-input" placeholder="Type a message…" rows="1"></textarea>
          <button id="chat-send" class="btn brand" title="Send">
    ...
    <div id="cover-progress" role="status" aria-live="polite">
      <div class="cp-text" id="cover-progress-text">
        <i class="fa-solid fa-spinner spin"></i> Fetching covers… <b id="cover-progress-count">0 / 0</b>
      </div>
    </div>

    <div id="loading-spinner" aria-label="Loading">
    ...
    <script type="module">
      ...
      function readURL() {
        ...
        sortMode = ["recent", "az", "za"].includes(s) ? s : "recent";
      ...
      function onKeyDownModal(e) {
        ...
        if (e.key === "/") {
          ...
          // other modals: don't trigger global "/" handler, but allow typing "/" into inputs
          e.stopPropagation();
          return;
        }
      ...
      function buildControls(row) {
        ...
        prog.innerHTML = `<i class="fa-solid fa-tv"></i> <span>${progressText(row)}</span>`;
        ...
        move.onchange = async (e) => {
          try {
            ...
            const { error } = await supabase.from("entries").update({ status: next }).eq("id", row.id);
            if (error) throw error;

            // local update (instant)
            const i = data.findIndex((r) => r.id === row.id);
            if (i >= 0) data[i] = { ...data[i], status: next };
      ...
      function render() {
        ...
        renderDebounce = setTimeout(() => {
          ...
          // Only show "No anime found" for SEARCH, not for empty tabs
      ...
      async function adjustEpisode(row, delta) {
        ...
        try {
          const { error } = await supabase.from("entries").update({ episode: next }).eq("id", row.id);
          if (error) throw error;

          // local update instantly
          const i = data.findIndex((r) => r.id === row.id);
          if (i >= 0) data[i] = { ...data[i], episode: next };
      ...
      async function loadEntries() {
        ...
        try {
          ...
          // background covers (do NOT block preloader)
      ...
      async function saveModal() {
        ...
        try {
          if (editingId) {
            const { data: upd, error } = await supabase.from("entries").update(row).eq("id", editingId).select().single();
            if (error) throw error;
            ...
            const { data: ins, error } = await supabase.from("entries").insert(row).select().single();
            if (error) throw error;
      ...
      function normalizeImport(o) {
        ...
        // Browser cannot use file:// covers
      ...
      function normalizeMAL(xmlText) {
        ...
        const mapKind = (t) => {
          const code = Number(t || 1); // 1=TV, 3=Movie, etc.
        ...
        if (Array.isArray(tryLists)) {
          for (const L of tryLists) {
            for (const e of L.entries || []) {
              const media = e.media || {};
              const title =
                media.title?.romaji || media.title?.english || media.title?.native || e.notes || "";
      ...
      // ---------- Scroll to top (robust with overflow-x hidden) ----------
  ...
  prefCover.addEventListener("change", async () => {
    try {
      ...
      function showCoverProgress() {
        ...
        coverProgressText.innerHTML = `<i class="fa-solid fa-spinner spin"></i> Fetching covers… <b id="cover-progress-count">${coverDone} / ${coverTotal}</b>`;
      }
      ...
      function finishCoverProgress() {
        ...
        coverProgressText.innerHTML = `<i class="fa-solid fa-circle-check"></i> Covers updated ✔`;
        setTimeout(() => {
      ...
      function enqueueMissingCovers(rows) {
        ...
        for (const r of list) {
          ...
          if (r.image_url) continue; // do not override existing covers
          if (coverAttemptedThisRun.has(r.id)) continue; // don't thrash
      ...
      async function processCoverRow(row) {
        ...
        // Only AniList provides alt titles; save when available
        if (r.titles) {
        ...
        // local update immediately (realtime will also sync)
      ...
      async function startCoverWorker() {
        ...
        try {
            ...
            while (coverWorkerActive < CONCURRENCY && coverQueue.size > 0) {
              ...
              (async () => {
                try {
                  ...
                  // ignore failures for now; user can click refresh later
                } finally {
                  coverDone++;
                  coverWorkerActive--;
                  updateCoverProgress();
          ...
          // reset per-run attempted set so future refresh can retry
          coverAttemptedThisRun.clear();

          // reset counters cleanly for next run
          ...
          // only show completion if we actually did work
      ...
      function updateSelfAvatar() {
        chatSelfAvatar.src = diceAvatar(getUsername());
      }
      ...
      async function loadChatHistory(limit = 3000) {
        ...
        try {
          ...
          chatMessagesEl.querySelectorAll(".msg").forEach((n) => n.remove());
      ...
      function renderChatMessage(msg) {
        ...
        avatar.alt = "";
        avatar.src = susie ? SUSIE_AVATAR : diceAvatar(msg.username || "bro");
      ...
      function scheduleTypingIndicatorUpdate() {
        ...
        typingUiDebounce = setTimeout(() => {
          ...
          if (count === 1) {
            ...
            chatTypingEl.textContent = `${only.username} is typing…`;
          } else {
            chatTypingEl.textContent = `${count} people are typing…`;
          }

          chatTypingEl.classList.add("visible");

          // schedule a follow-up cleanup so it disappears smoothly without flicker
      ...
      async function sendChat() {
        ...
        // Start cooldown immediately (client), server also enforces
      ...
      chatBottomFab.addEventListener("click", () => scrollChatToBottom(true));

      chatMessagesEl.addEventListener(
        "scroll",
        () => {
          const dist = chatMessagesEl.scrollHeight - chatMessagesEl.scrollTop - chatMessagesEl.clientHeight;
          if (dist > 100) chatBottomFab.classList.add("visible");
      ...
      function openChat() {
        ...
        loadChatHistory().then(() => scrollChatToBottom(true));
        setupChatRealtime();
        setupTyping();
      ...
      // ---------- Init + Preloader (FIXED: no more stuck-on-reload) ----------
      async function getSessionWithTimeout(timeoutMs = 2500) {
        const timeout = new Promise((resolve) =>
          setTimeout(() => resolve({ session: null, timedOut: true }), timeoutMs)
        );
        ...
        const attempt = (async () => {
          try {
            ...
            return { session: null, timedOut: false, error: e };
          }
        })();
      ...
      function startAuthListener() {
        ...
        supabase.auth.onAuthStateChange(async (event, s) => {
          // initApp already handled INITIAL_SESSION (prevents double-load + race issues)
      ...
      async function initApp() {
        ...
        try {
          const r = await getSessionWithTimeout(2500);
          if (r.timedOut) {
            console.warn("Supabase getSession timed out; continuing without blocking UI.");
          }
          if (r.error) {
            console.warn("Supabase getSession error:", r.error);
          }
